...
test('mobile styles drawer keeps a large, interactive video and restores normal navigation', async ({ page, browserName }) => {
  ...
  const renderedLines = () => overlay.evaluate(el => Math.round(el.getBoundingClientRect().height / parseFloat(getComputedStyle(el).lineHeight)));
  expect(await renderedLines()).toBe(await overlay.evaluate(el => el.textContent!.split('\n').length));
  await fontSize.click();
  await page.getByRole('option', { name: '240', exact: true }).click();
  await expect(fontSize).toHaveText('240');
  await expect.poll(() => overlay.evaluate(el => parseFloat(getComputedStyle(el).fontSize))).toBeCloseTo(defaultSize * 240 / autoSize, 1);
  await fontSize.click();
  await page.getByRole('option', { name: /^אוטומטי/ }).click();
